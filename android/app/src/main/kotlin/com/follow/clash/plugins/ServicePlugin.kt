...
class ServicePlugin : FlutterPlugin, MethodChannel.MethodCallHandler,
    ...
    private fun resultSuccessOnMainThread(result: MethodChannel.Result, value: Any?) {
        launch(Dispatchers.Main) { result.success(value) }
    }
    ...
    private fun handleInvokeAction(call: MethodCall, result: MethodChannel.Result) {
        launch {
            ...
            val r = Service.invokeAction(data) {
                resultSuccessOnMainThread(result, it)
            }
            if (r.isFailure) {
                // Return null so Dart can treat it as "no result" (and avoid JSON decode crashes).
                resultSuccessOnMainThread(result, null)
            }
        }
    }
    ...
    private fun handleSyncState(call: MethodCall, result: MethodChannel.Result) {
        ...
        launch {
            State.syncState()
            resultSuccessOnMainThread(result, "")
        }
    }
    ...
    fun handleInit(result: MethodChannel.Result) {
        ...
        launch {
            Service.setEventListener {
                ...
                resultSuccessOnMainThread(result, it.message)
            }
    ...
    private fun handleGetRunTime(result: MethodChannel.Result) {
        launch {
            ...
            resultSuccessOnMainThread(result, State.runTime)
        }
    }
}
