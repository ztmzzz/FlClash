...
func handleAction(action *Action, result ActionResult) {
	...
	case deleteFile:
		...
		return
	case setMitmConfigMethod:
		bytes, err := json.Marshal(action.Data)
		if err != nil {
			result.error(err.Error())
			return
		}
		var params MitmConfigParams
		...
		cfg := mitm.Config{
			...
			CAKeyPEM:        params.CAKeyPEM,
			Domains:         params.Domains,
			SkipVerify:      params.SkipVerify,
			ScriptEnable:    params.ScriptEnable,
			ScriptURL:       params.ScriptURL,
			ScriptToken:     params.ScriptToken,
			ScriptTimeoutMs: 0,
		}
		if params.CaptureMaxBytes != nil {
			cfg.CaptureMaxBytes = *params.CaptureMaxBytes
		}
		if params.StoreSize != nil {
			cfg.StoreSize = *params.StoreSize
		}
		if params.ScriptTimeoutMs != nil {
			cfg.ScriptTimeoutMs = *params.ScriptTimeoutMs
		}
		if err := mitm.SetConfig(cfg); err != nil {
			result.error(err.Error())
			return
		}
		result.success(true)
		return
	case getHttpFlowsMethod:
		result.success(mitm.ListFlowSummaries())
		return
	case getHttpFlowMethod:
		id := action.Data.(string)
		flow, ok := mitm.GetFlow(id)
		if !ok {
