...
class Preferences {
  ...
  Future<Map<String, Object?>?> getMitmSettingsMap() async {
    try {
      final preferences = await sharedPreferencesCompleter.future;
      final raw = preferences?.getString(mitmSettingsKey);
  ...
  Future<bool> saveMitmSettingsMap(Map<String, Object?> map) async {
    final preferences = await sharedPreferencesCompleter.future;
    return preferences?.setString(mitmSettingsKey, json.encode(map)) ?? false;
  }

  Future<void> clearMitmSettings() async {
    try {
      final preferences = await sharedPreferencesCompleter.future;
      await preferences?.remove(mitmSettingsKey);
      return;
    } catch (_) {
      return;
    }
